...
def calculate_distance(path):
	...
	for i in range(NUMBER_OF_LOCATIONS):
		if i == NUMBER_OF_LOCATIONS-1:
			location_i = locations[path.locations.index(i+1)]
...
def generate_offspring(parent_i):
	...
	for l in range(0,POP_SIZE//4):
		...
		for i in range(rand1, rand2):
			offspring_i.insert(i, aux[i-rand1])
		index_worst = find_worst_individuals()
		offspring = Individual(offspring_i)
		"""if offspring.distance < parent_j.distance:
			index_parent = individuals.index(parent_j)
			individuals[index_parent] = offspring
			children.append(index_parent)"""
		individuals[index_worst] = offspring
		children.append(index_worst)
	return children	
...
def mutate_population(children):
	...
	for i in range(len(children)):
		for j in range(0,1):
			...
			aux = individuals[children[i]].locations[rand1]
			individuals[children[i]].locations[rand1] = individuals[children[i]].locations[rand2]
			individuals[children[i]].locations[rand2] = aux

# Main function.

read_file()
grapth_distances = [] #type: list
grapth_generations = [] #type: list
...
while True:
	...
	if best.distance < best_distance_yet:
		...
		print(generations)
		grapth_generations.append(generations)
		grapth_distances.append(best_distance_yet)
		counter = 0
...
plt.plot(grapth_generations, grapth_distances)
